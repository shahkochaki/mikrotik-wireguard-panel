...
$user = getUserById($id);
if (!$user) {
    flashSet('danger', __('user_not_found'));
    header('Location: users');
    exit;
}

$pageTitle = __('page_edit_user') . $user['name'];
$errors    = [];

if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!validateCsrf()) die(__('invalid_request'));
    ...
    if ($expiryDate) {
        ...
        if (!$expiry) $errors[] = __('err_expiry_invalid');
    }
    ...
    if (empty($errors)) {
        try {
            ...
            // Reload
            $user = getUserById($id);
            flashSet('success', __('flash_user_updated'));
        } catch (Throwable $e) {
...
<div class="row g-4">
    ...
    <div class="col-lg-8">
        <div class="card border-0 shadow-sm">
            <div class="card-header bg-transparent">
                <h6 class="mb-0 fw-semibold"><i class="fas fa-pen me-2"></i><?= __('page_edit_user') ?><?= e($user['name']) ?></h6>
            </div>
            <div class="card-body">
                <form method="POST" action="" novalidate>
                    ...
                    <div class="row g-3">
                        ...
                        <div class="col-md-6">
                            <label class="form-label"><?= __('lbl_username') ?></label>
                        ...
                        <div class="col-md-6">
                            <label class="form-label"><i class="fas fa-calendar me-1"></i><?= __('lbl_expiry_date') ?></label>
                        ...
                        <div class="col-12">
                            <div class="form-check form-switch">
                                ...
                                <label class="form-check-label" for="isActive"><?= __('lbl_user_active') ?></label>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="d-flex gap-2 mt-4">
                        <button type="submit" class="btn btn-primary">
                            <i class="fas fa-save me-2"></i><?= __('btn_save_changes') ?>
                        </button>
                        <a href="users" class="btn btn-outline-secondary"><?= __('back') ?></a>
                    </div>
                </form>
            </div>
        </div>
    ...
    <div class="col-lg-4">
        <div class="card border-0 shadow-sm mb-3">
            <div class="card-header bg-transparent">
                <h6 class="mb-0 fw-semibold"><i class="fas fa-info-circle me-2"></i><?= __('card_tech_info') ?></h6>
            </div>
            <div class="card-body small">
                <dl class="row mb-0">
                    <dt class="col-5"><?= __('col_ip') ?></dt>
                    ...
                    <dt class="col-5"><?= __('lbl_peer_id') ?></dt>
                    <dd class="col-7"><code><?= e($user['mikrotik_peer_id'] ?? '—') ?></code></dd>
                    <dt class="col-5"><?= __('lbl_queue_id') ?></dt>
                    ...
                    ?>
                    <dt class="col-5"><?= __('col_download') ?></dt>
                    <dd class="col-7"><?= formatBytes($rx) ?></dd>
                    <dt class="col-5"><?= __('col_upload') ?></dt>
